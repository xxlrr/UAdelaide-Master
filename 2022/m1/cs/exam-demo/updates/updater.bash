...
create_file()
{
    ...
    then
        ...
        then
            ...
            cp "${src}" "${dst}"
            if [ "${src}" == "./updates/installer.bash" ]
            then
                echo "Updated the installer.bash - please rerun make"
                kill -1 ${PPID}
                exit 0
            fi
        fi
    ...
    while [ "${#@}" -gt 0 ] ; do
        ...
        case "${src}" in
            "./gMakefile")
                ...
                ;;
            "./Makefile")
                create_file ./Makefile "${UPDATES_DIR}/Makefile" Makefile update
                ;;
            "./README")
                create_file ./README "${UPDATES_DIR}/README" README update
                ;;
            "./updater.bash" | "./startups-datestamp")
                # ignore these
                ;;
            "./updates/log")
                # create but do not update
                create_file "${src}" "${dst}" "${displayname}" donottouch
                ;;
            *)
...
fix_executable()
{
    while [ "${#@}" -gt 0 ] ; do
        ...
        *.cpp | *.h | *.tst | *.cmp | *.hdl | *.jack | *.php | *.c | *.o | *.a | \
        Makefile | gMakefile | README | startups-datestamp)
            ;;
        *)
...
# if not previously installed - remove all quarantine flags
if [ ! -f "${INSTALLED}" ]
then
    fix_quarantine
fi
