...
public class Main {
    ...
    /**
     * Get a int value from the input. If the input is not a int, 
     * the function will ask the user to enter again.
     * 
    ...
    public static int getIntInput(Scanner inputScan, String question){
        ...
        while(true) {
            try{
                String _input = getInput(inputScan, question);
                _number = Integer.parseInt(_input);
                break;
            }
            catch(NumberFormatException e) {
                System.out.println("Error! The input is invalid, try again. ");
            }
        }
        return _number;
    }
    ...
    /**
     * Get a int value from the input. If the input is invalid, 
     * the function will ask the user to enter again.
     * 
     * @return the int value entered by the user
     */
    public static int getIntInput(Scanner inputScan, String question, int min, int max){
        int _number;
        while(true) {
            _number = getIntInput(inputScan, question);
            if(_number >= min && _number <= max) {
                break;
            }
            System.out.println("Error! The input is invalid,"
                    + " only accept (" + min + "-" + max + ") as legal input,"
                    + " please try again. ");
        }
        return _number;
    }
    ...
    public static boolean performAction(ServiceCenter serviceCenter, int _option, Scanner inputScan){
        ...
        switch(_option){
            case 1:
                ...
                int triageLevel = getIntInput(inputScan, "Please enter the triage level of patient: ", 1, 5);
            ...
            case 4:
                ...
                id = getIntInput(inputScan, "Please enter the id of patient: ");
                serviceCenter.assignAmbulanceForPatient(id);
                waitInput(inputScan);
                break;
            case 5:
                clearScreen();
                ...
                id = getIntInput(inputScan, "Please enter the id of patient: ");
                serviceCenter.checkPositionForPatient(id);
                waitInput(inputScan);
                break;
            case 0:
                exitMessage();
